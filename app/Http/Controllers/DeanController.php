...
class DeanController extends Controller
{
    ...
    public function index(Request $request)
    {
      ...
      /**
       * 
       * APPROVED DOCUMENTS BASED ON LOGGNED IN USER AND MUST BE A DEAN 
       * 
       * SO FILTER WITH   faculty_code = A, I 
       * 
       * THEN AFTER GETTING THE RESULT YOU CAN THE FILTER BY DEPT_CODE TO SHOW RECORDS IN DIFFERENT DEPARTMENTS
       */
        $level_id = 3;
      ...
      if($request->ajax())
      {
        ...
        try {
            ...
            if ($existingApproval) {
                ...
                $existingApproval->acted_on_by = $deanID;
                $existingApproval->date_acted_upon = $date;
            ...
            // Retrieve existing record from the ApprovalHistory table
            $existingHistory = ApprovalHistory::where('file_id', $file_id)->first();
            //$data['sssss'] =$existingHistory;
            if ($existingHistory) {
                // Clone the existing history record and update its attributes
                $newHistory = $existingHistory->replicate(); // Clone the existing record
                //$newHistory->file_id = $randomId;
                $newHistory->level_id = 5;
                $newHistory->date_acted_upon = $date;
                $newHistory->acted_on_by = $deanID;
